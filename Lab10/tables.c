...
int add_var(SymTable* st, char* s, int line, type_names type) {
...
type_names get_type(SymTable* st, int i) {
    return st->t[i].type;
}

void print_sym_table(SymTable* st) {
    const char* _type_strings[4] = {"INT", "REAL", "STRING", "BOOL"};
    printf("Symbols table:\n");
    for (int i = 0; i < st->size; i++) {
         printf("Entry %d -- name: %s, line: %d, type: %s\n",
                i, get_name(st, i), get_line(st, i), _type_strings[get_type(st, i)]);
    }
}
